...
3. ~~**#437**~~ — **measured on 2026-09-11 and it does not hold.** The round ran exactly as scoped: six fixtures, both prompt routes, bars declared before the first candidate call, 252 outputs. **The clause does not ship and Normal polish keeps its single block** — which the issue declares a legitimate close, and it is written up with the run counts in `docs/research/437-longform-breaks/findings.md`. Three things a reader should not rediscover. **The system prompt is not the lever**: the licence written as a permission, as an imperative, with worked examples, restated in the GOAL line, and asking for a plain newline instead of the marker produced **0 line breaks in 132 outputs**. **The user turn is a lever and a weak one**: about one output in four, unstably, and twice in fifteen it breaks after every sentence. **The missing capability is discourse-boundary detection, not fidelity**: handed already-polished text with the single job of adding breaks, Apple FM returns whitespace-stripped output byte-identical to its input 30/30, with no list syntax anywhere, and puts the breaks in the wrong places — one per sentence on a three-section explanation, and inside a single idea on the control fixture. The list risk that motivated the whole careful framing never materialised: **0 list markers in 432 outputs**. `PolishPromptParityTests` now pins the ban, and ADR 0003 carries the amendment.
