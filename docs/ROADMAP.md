...
| `vocabulary` | Built — #80 shipped in PR #525, device-validated in the app and the keyboard. Its glossary half is #536. |
...
7. **#536** — the polish glossary has no measurable effect: two device captures show Apple FM ignoring our term list entirely. It carries a pre-registered experiment against `n0an/VivaDicta`, which ships the same two layers and frames its glossary as *phonetic repair* rather than *spelling preservation*. Two outcomes, both actionable: rewrite the framing, or cut the glossary and stop claiming a term with no variants does anything. **Settles #80's decision 6, the paywall sentence, which is why it sits here and not in 2.1.**
