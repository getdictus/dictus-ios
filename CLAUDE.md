...
- Un worktree neuf — comme tout "Reset Package Caches" — résout les packages de zéro : `xcodebuild -resolvePackageDependencies -project Dictus.xcodeproj -scheme DictusApp -derivedDataPath build/DerivedData`, puis le build. Aucune étape de patch : depuis FluidAudio 0.15.7 (#558) le paquet compile tel quel sous Xcode 26.4.1 / Swift 6.3.
- Un checkout qui a déjà résolu FluidAudio 0.12.x échoue à la résolution sur 0.15.7 (`Disabled default traits … on package 'fluidaudio' (FluidAudio) that declares no traits`, mesuré sur le worktree `device` le 2026-09-14) : la clé `traits = ()` du pbxproj (#558) ne s'applique pas à l'ancien checkout. Supprimer `SourcePackages` sous le `-derivedDataPath` du build (ou « Reset Package Caches » dans Xcode), puis résoudre à nouveau. Le build demande Xcode 26.4.1 ou plus récent : Xcode 26.3 ignore cette clé, et le link x86_64 du simulateur échoue.
